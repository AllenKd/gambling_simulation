...
class Simulator(object):
    ...
    def write_to_db(self, battled_player):
        self.logger.info('start write to db: {}'.format(self.config[constant.DB][constant.schema]))
        battled_player.battle_statistic.to_sql(con=self.engine, name='player_{}'.format(battled_player.id),
                                               if_exists='replace', schema=self.config[constant.DB][constant.schema],
                                               index_label='run')

        summarize_row = pd.DataFrame({k: v for k, v in battled_player.battle_summarize.items()}, index=[0])

        summarize_row.to_sql(con=self.engine, name=constant.player_summarize, if_exists='append',
                             schema=self.config[constant.DB][constant.schema], index=False)
    ...
    def summarize_gambling(self):
        ...
        for player in self.players:
            strategies[player.strategy_name][constant.win_player_ratio] += (player.battle_summarize[
                constant.final_result]) / self.player_strategy[player.strategy_name]

            strategies[player.strategy_name][constant.average_player_win] += (player.battle_summarize[
                                                                                  constant.final_money] - self.player_init_money) / \
